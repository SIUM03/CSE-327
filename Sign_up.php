...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $customer_sql = "INSERT INTO Customer (name, nid, phone, email, address, password, kycstatus) 
                   VALUES ('$name', '$nid', '$phone', '$email', '$address', '$password', 0)";

    if ($conn->query($customer_sql) === TRUE) {
        $customerId = $conn->insert_id;

        $account_sql = "INSERT INTO Account (type, status, Balance, customerId) 
                    VALUES ('$type', 'active', '$amount', '$customerId')";
